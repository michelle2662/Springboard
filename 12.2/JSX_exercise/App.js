function App ()
{
	...
	return (
		<div>
			...
			<ul>
				{spacePhenomena.map(space => {
					const randomIndex = Math.floor(Math.random() * observationStatuses.length)
					const spacePhenomenaStatus = observationStatuses[randomIndex]
					console.log(spacePhenomenaStatus)

					return (
						<li key = {space.id}>
							{space.emoji} {space.name} - {spacePhenomenaStatus}
							{randomIndex == 2 ? <span> 💡 Gear up with your best equipment! </span> :null}
						</li>
					)
